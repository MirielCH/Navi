...
COMMAND_DUNGEON: re.Pattern = re.compile(rf"\bdung(?:eon)?\b")
COMMAND_DUNGEON_MINIBOSS_MININTBOSS: re.Pattern = re.compile(rf"(?:\bdung\b|\bdungeon\b|\bminiboss\b|\bminintboss\b)")
COMMAND_EGG_BUY_EASTER_BOOST: re.Pattern = re.compile(rf"(?:\begg\b|\beaster\b)\s+\bbuy\b\s+\beaster\b\s+\bboost\b")
COMMAND_EGG_GOD: re.Pattern = re.compile(rf"(?:\begg\b|\beaster\b)\s+\bgod\b")
COMMAND_EGG_USE_SQUARE_EGG: re.Pattern = re.compile(rf"(?:\begg\b|\beaster\b)\s+\buse\b\s+\bsquare\b\s+\begg\b")
...
COMMAND_SMR_BUY_SUMMER_BOOST: re.Pattern = re.compile(rf"(?:\bsummer\b|\bsmr\b)\s+\bbuy\b\s+\bsummer\b\s+\bboost\b")
COMMAND_SMR_COLOR_TOURNAMENT: re.Pattern = re.compile(rf"(?:\bsummer\b|\bsmr\b)\s+\bcolor\b\s+\btournament\b")
COMMAND_SMR_SURF: re.Pattern = re.compile(rf"(?:\bsmr\b|\bsummer\b)\s+\bsurf\b")
COMMAND_SMR_USE_DRINK: re.Pattern = re.compile(rf"(?:\bsummer\b|\bsmr\b)\s+(\bdrink\b|\buse\b)\s+(\bblue\b|\bgreen\b|\bpink\b|\byellow\b)\s+\bdrink\b")
...
COMMAND_USE_TIME_COOKIE: re.Pattern = re.compile(rf"use\s+(?:\btime\b\s+\bcookie\b|\btc\b)")
