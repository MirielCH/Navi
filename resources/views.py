...
class ReadyView(discord.ui.DesignerView):
    ...
    def __init__(self, bot: bridge.AutoShardedBot, ctx: Union[commands.Context, discord.ApplicationContext], user: discord.User,
        ...
        if not user_mentioned and user_settings.alts:
            self.add_item(discord.ui.ActionRow(components.SwitchReadyAltSelect(self)))
        ...
        self.add_item(discord.ui.ActionRow(components.ToggleAutoReadyButton(custom_id=custom_id, label=label)))
        if isinstance(ctx, discord.ApplicationContext):
            self.add_item(discord.ui.ActionRow(components.CustomButton(discord.ButtonStyle.grey, 'show_settings', '➜ Settings')))
...
class ConfirmUserView(discord.ui.DesignerView):
    ...
    def __init__(self, asked_user: discord.User, confirm_label: Optional[str] = 'Yes',
        ...
        self.add_item(
            discord.ui.ActionRow(
                components.CustomButton(style=discord.ButtonStyle.green, custom_id='confirm', label=confirm_label),
                components.CustomButton(style=discord.ButtonStyle.grey, custom_id='cancel', label=cancel_label)
            )
        )
    async def interaction_check(self, interaction: discord.Interaction):
...
class SettingsAltsView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, user_settings: users.User,
        ...
        self.user_settings = user_settings
        self.embed_function = embed_function
        disabled_add = True if len(user_settings.alts) >= 24 else False
        disabled_remove = True if not user_settings.alts else False
        self.add_item(discord.ui.ActionRow(components.AddAltSelect(self, disabled=disabled_add)))
        self.add_item(discord.ui.ActionRow(components.RemoveAltSelect(self, disabled=disabled_remove)))
...
class SettingsClanView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, clan_settings: clans.Clan,
        ...
        self.add_item(discord.ui.ActionRow(components.ManageClanSettingsSelect(self)))
        self.add_item(discord.ui.ActionRow(
                components.SwitchSettingsSelect(self, COMMANDS_SETTINGS)
            )
        )
...
class SettingsHelpersView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, user_settings: users.User,
        ...
        self.add_item(
            discord.ui.ActionRow(
                components.ToggleUserSettingsSelect(self, toggled_settings, 'Toggle helpers')
            )
        )
        self.add_item(discord.ui.ActionRow(components.SetFarmHelperModeSelect(self)))
        self.add_item(discord.ui.ActionRow(components.ManageHelperSettingsSelect(self)))
        self.add_item(discord.ui.ActionRow(components.SwitchSettingsSelect(self, COMMANDS_SETTINGS)))
...
class SettingsPortalsView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, user_settings: users.User,
        ...
        self.add_item(discord.ui.ActionRow(components.ManagePortalsSelect(self)))
        self.add_item(discord.ui.ActionRow(components.ManagePortalSettingsSelect(self)))
        self.add_item(discord.ui.ActionRow(components.SwitchSettingsSelect(self, COMMANDS_SETTINGS)))
...
class SettingsReadyView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, user_settings: users.User,
        ...
        self.add_item(discord.ui.ActionRow(components.ManageReadySettingsSelect(self)))
        self.add_item(discord.ui.ActionRow(components.ToggleReadySettingsSelect(self, toggled_settings_other, 'Toggle other commands',
                                                           'toggle_other_commands')))
        self.add_item(discord.ui.ActionRow(components.SwitchToReadyRemindersSelect(self)))
        self.add_item(discord.ui.ActionRow(components.SwitchSettingsSelect(self, COMMANDS_SETTINGS)))
...
class SettingsReadyRemindersView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, user_settings: users.User,
        ...
        self.clan_settings = clan_settings
        self.embed_function = embed_function
        self.ready_view = SettingsReadyView
        ...
        self.add_item(
            discord.ui.ActionRow(
            components.ToggleReadySettingsSelect(self, toggled_settings_commands, 'Toggle command reminders',
                                                'toggle_command_reminders')
            )
        )
        self.add_item(
            discord.ui.ActionRow(
                components.ToggleReadySettingsSelect(self, toggled_settings_events, 'Toggle event reminders',
        ...
        if toggled_settings_commands:
            self.add_item(
                discord.ui.ActionRow(
                    components.ToggleReadySettingsSelect(self, toggled_settings_seasonal, 'Toggle seasonal reminders',
                                                           'toggle_seasonal_reminders')
                )
            )
        self.add_item(discord.ui.ActionRow(components.ManageReadyReminderChannelsSelect(self)))
        self.add_item(discord.ui.ActionRow(components.ReadyGoBackButton(self, 'reminders_back', '< Back')))
...
class SettingsMultipliersView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, user_settings: users.User,
        ...
        self.user_settings = user_settings
        self.embed_function = embed_function
        self.add_item(discord.ui.ActionRow(components.ManageMultiplierSettingsSelect(self)))
        select_disabled: bool = False
        if user_settings.multiplier_management_enabled and user_settings.current_area != 20:
            select_disabled = True
        self.add_item(discord.ui.ActionRow(components.ManageManagedMultipliersSelect(self, disabled=select_disabled)))
        self.add_item(discord.ui.ActionRow(components.ManageManualMultipliersSelect(self)))
        self.add_item(discord.ui.ActionRow(components.SwitchSettingsSelect(self, COMMANDS_SETTINGS)))
...
class SettingsRemindersView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, user_settings: users.User,
        ...
        self.add_item(discord.ui.ActionRow(components.ManageReminderBehaviourSelect(self)))
        self.add_item(discord.ui.ActionRow(components.ToggleUserSettingsSelect(self, toggled_settings_commands_1, 'Toggle reminders (I)',
                                                                          'toggle_command_reminders_1')))
        self.add_item(discord.ui.ActionRow(components.ToggleUserSettingsSelect(self, toggled_settings_commands_2, 'Toggle reminders (II)',
                                                                          'toggle_command_reminders_2')))
        self.add_item(discord.ui.ActionRow(components.ToggleUserSettingsSelect(self, toggled_settings_commands_3, 'Toggle reminders (III)',
...
class SettingsReminders2View(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, user_settings: users.User,
        ...
        self.add_item(discord.ui.ActionRow(components.ToggleUserSettingsSelect(self, toggled_settings_events, 'Toggle event reminders',
                                                          'toggle_event_reminders')))
        if toggled_settings_seasonal:
            self.add_item(discord.ui.ActionRow(components.ToggleUserSettingsSelect(self, toggled_settings_seasonal, 'Toggle seasonal reminders',
                                                              'toggle_seasonal_reminders')))
        self.add_item(discord.ui.ActionRow(components.SwitchSettingsSelect(self, COMMANDS_SETTINGS)))
...
class SettingsUserView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, user_settings: users.User,
        ...
        self.add_item(discord.ui.ActionRow(components.SetDonorTierSelect(self, 'Change partner donor tier', 'partner', partner_select_disabled)))
        self.add_item(discord.ui.ActionRow(components.SwitchSettingsSelect(self, COMMANDS_SETTINGS)))
...
class SettingsMessagesView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, user_settings: users.User,
        ...
        self.add_item(
            discord.ui.ActionRow(
                components.SetReminderMessageButton(style=discord.ButtonStyle.blurple, custom_id='set_message',
                                                            label='Change', disabled=change_disabled),
           components.SetReminderMessageButton(style=discord.ButtonStyle.red, custom_id='reset_message',
                                                            label='Reset')
            )
        )
...
class SettingsPartnerView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, user_settings: users.User,
        ...
        self.embed_function = embed_function
        self.add_item(discord.ui.ActionRow(components.AddPartnerSelect(self, "Change partner")))
        self.add_item(discord.ui.ActionRow(components.ManagePartnerSettingsSelect(self)))
        self.add_item(discord.ui.ActionRow(components.SetPartnerAlertThreshold(self)))
        partner_select_disabled = True if user_settings.partner_id is not None else False
        self.add_item(discord.ui.ActionRow(components.SetDonorTierSelect(self, 'Change partner donor tier', 'partner', partner_select_disabled)))
        self.add_item(discord.ui.ActionRow(components.SwitchSettingsSelect(self, COMMANDS_SETTINGS)))
...
class RemindersListView(discord.ui.DesignerView):
    ...
    def __init__(self, bot: bridge.AutoShardedBot, ctx: Union[commands.Context, discord.ApplicationContext], user: discord.User,
        ...
        self.active_alt_id = user.id
        if not user_mentioned and user_settings.alts:
            self.add_item(discord.ui.ActionRow(components.SwitchRemindersListAltSelect(self)))
        if custom_reminders:
            self.add_item(discord.ui.ActionRow(components.DeleteCustomReminderSelect(self, custom_reminders)))
...
class StatsView(discord.ui.DesignerView):
    ...
    def __init__(self, bot: bridge.AutoShardedBot, ctx: Union[commands.Context, discord.ApplicationContext],
        ...
        if not user_mentioned and user_settings.alts:
            self.add_item(discord.ui.ActionRow(components.SwitchStatsAltSelect(self)))
...
class SettingsServerAutoFlexView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, guild_settings: guilds.Guild,
        ...
        self.add_item(discord.ui.ActionRow(components.ManageServerSettingsAutoFlexSelect(self)))
        self.add_item(discord.ui.ActionRow(components.ToggleServerSettingsSelect(self, toggled_auto_flex_alerts_1,
                                                            'Toggle auto flex alerts (I)',
                                                            'toggle_auto_flex_alerts_1')))
        self.add_item(discord.ui.ActionRow(components.ToggleServerSettingsSelect(self, toggled_auto_flex_alerts_2,
                                                            'Toggle auto flex alerts (II)',
                                                            'toggle_auto_flex_alerts_2')))
        self.add_item(discord.ui.ActionRow(components.ToggleServerSettingsSelect(self, toggled_auto_flex_alerts_3,
                                                             'Toggle auto flex alerts (III)',
                                                            'toggle_auto_flex_alerts_3')))
        self.add_item(discord.ui.ActionRow(components.SwitchSettingsSelect(self, COMMANDS_SERVER_SETTINGS)))
...
class SettingsServerAutoFlex2View(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, guild_settings: guilds.Guild,
        ...
        self.add_item(discord.ui.ActionRow(components.ToggleServerSettingsSelect(self, toggled_auto_flex_alerts_3,
                                                            'Toggle auto flex alerts (III)',
                                                            'toggle_auto_flex_alerts_3')))
        self.add_item(discord.ui.ActionRow(components.ToggleServerSettingsSelect(self, toggled_auto_flex_alerts_seasonal,
...
class SettingsServerEventPingsView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, guild_settings: guilds.Guild,
        ...
        self.add_item(discord.ui.ActionRow(components.ToggleEventPingsSelect(self, toggled_events,
                                                             'Toggle event pings',
                                                             'toggle_event_pings')))
        self.add_item(discord.ui.ActionRow(components.ManageEventPingMessagesSelect(self,)))
        self.add_item(discord.ui.ActionRow(components.SwitchSettingsSelect(self, COMMANDS_SERVER_SETTINGS)))
...
class DevEventReductionsView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, all_cooldowns: list[cooldowns.Cooldown],
        ...
        self.add_item(
            discord.ui.ActionRow(
                components.CopyEventReductionsButton(discord.ButtonStyle.grey, 'copy_slash_text', 'Copy slash > text'),
                components.CopyEventReductionsButton(discord.ButtonStyle.grey, 'copy_text_slash', 'Copy text > slash')
            )
        )
...
class DevSeasonalEventView(discord.ui.DesignerView):
    ...
    def __init__(self, ctx: bridge.BridgeContext, bot: bridge.AutoShardedBot, active_event: str,
        ...
        self.user = ctx.author
        self.embed_function = embed_function
        self.add_item(discord.ui.ActionRow(components.SetSeasonalEventSelect(seasonal_events, active_event,
                                            'Set seasonal event ...')))
