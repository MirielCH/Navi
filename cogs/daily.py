...
class DailyCog(commands.Cog):
    ...
    async def get_daily_message(self, ctx: commands.Context) -> Tuple[discord.Message, str]:
        """Waits for the daily message in the channel and returns it if found."""
        def epic_rpg_check(m: discord.Message) -> bool:
            correct_message = False
            try:
                ctx_author = str(ctx.author.name).encode('unicode-escape',errors='ignore').decode('ASCII').replace('\\','')
                message = functions.encode_message_non_async(m)
                if settings.DEBUG_MODE: logs.logger.debug(f'Daily detection: {message}')
    ...
    async def daily(self, ctx: commands.Context) -> None:
        """Detects EPIC RPG daily messages and creates reminders"""
        prefix = ctx.prefix
        ...
        try:
            ...
            if bot_message.find(f'\'s cooldown') > 1:
                ...
                reminder: reminders.Reminder = (
                    await reminders.insert_user_reminder(self.bot, ctx.author.id, 'daily', time_left,
            ...
            logs.logger.error(f'Daily detection error: {e}')
            return
