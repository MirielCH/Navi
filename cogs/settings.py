...
from resources import components, emojis, exceptions, functions, settings, strings


class SettingsGuildView(discord.ui.View):
    """View with a all components to manage guild settings.
    ...
    None otherwise.
    """
    def __init__(self, ctx: discord.ApplicationContext, user_settings: users.User,
    ...
    @discord.ui.button(custom_id="Reset guild channel", style=discord.ButtonStyle.red, label='Reset channel', row=3)
    async def button_abort(self, button: discord.ui.Button, interaction: discord.Interaction):
        """Reset guild channel button"""
        self.value = button.custom_id
        self.stop()
